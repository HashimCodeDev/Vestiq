...
jest.mock('@/hooks/useUserInfo', () => ({
  /**
   * Mock implementation of the useUserInfo hook that returns a fixed user object.
   * Used in integration tests to avoid having to deal with the complexities of
   ...
   * @property {boolean} loading Whether user data is currently loading.
   */
  useUserInfo: () => ({
...
jest.mock('@/components/authentication/ProtectedRoute', () => {
  /**
   * A mock implementation of the ProtectedRoute component.
   *
   * Used in integration tests to provide a fake ProtectedRoute component that
   * renders its children directly, without performing any authentication checks.
   *
   * @param {React.ReactNode} children The children that will be rendered.
   * @returns {React.ReactElement} A React element containing the children.
   */
...
describe('User Flow Integration Tests', () => {
  ...
  describe('Home Page User Journey', () => {
    it('should allow user to navigate through quick actions', async () => {
      ...
      const outfitBuilderLink = screen.getByRole('link', {
        name: /outfit builder/i,
      });
    ...
    it('should display user information correctly', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Good Morning, Test User/i),
        ).toBeInTheDocument();
      });

      // Check that user level is displayed
      expect(
        screen.getByRole('status', { name: /User level/i }),
      ).toBeInTheDocument();

      // Check that profile link is present
      const profileLink = screen.getByRole('link', {
        name: /Go to Test User profile/i,
      });
  ...
  describe('Challenge Interaction Flow', () => {
    ...
    it('should allow interaction with challenge cards', async () => {
      ...
      // Test that challenge section is present
      const challengeSection = screen
        .getByText('Active Challenges')
        .closest('section');
      expect(challengeSection).toBeInTheDocument();

      // Test that challenge cards are interactive (check for clickable elements)
      const challengeLinks = screen.getAllByRole('link');
      const challengeButtons = screen.getAllByRole('button');
      expect(challengeLinks.length + challengeButtons.length).toBeGreaterThan(
        0,
      );
    });
  });
