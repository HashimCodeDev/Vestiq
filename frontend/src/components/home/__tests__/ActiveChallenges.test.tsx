...
describe('ActiveChallenges Component', () => {
  it('renders the main heading', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Active Challenges' })).toBeInTheDocument()
  })
  ...
  it('displays challenge descriptions', () => {
    ...
    expect(screen.getByText('Create 7 outfits using only 10 pieces')).toBeInTheDocument()
    expect(screen.getByText('Incorporate one bright color daily')).toBeInTheDocument()
    expect(screen.getByText('Style retro pieces for modern looks')).toBeInTheDocument()
  })
  ...
  it('displays progress information', () => {
    ...
    expect(screen.getByText('1/5 outfits')).toBeInTheDocument()
  })
  ...
  it('renders progress bars with correct values', () => {
    ...
    expect(progressBars[0]).toHaveAttribute('aria-valuenow', '43') // 3/7 ≈ 43%
    expect(progressBars[1]).toHaveAttribute('aria-valuenow', '29') // 2/7 ≈ 29%
    expect(progressBars[2]).toHaveAttribute('aria-valuenow', '20') // 1/5 = 20%
  })

  it('has proper semantic structure', () => {
    render(<ActiveChallenges />)
    
    expect(screen.getByRole('region', { name: 'Active challenges' })).toBeInTheDocument()
  ...
  it('maintains proper spacing and layout', () => {
    ...
    const challengesContainer = screen.getByRole('region', { name: 'Active challenges' })
